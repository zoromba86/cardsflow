...
export function UnloadModal({ open, balance, ccy, onConfirm, onClose }: UnloadModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    if (isNaN(num) || num <= 0 || num > balNum) return;
    setLoading(true);
    try {
      await onConfirm(num);
      setSuccess(true);
        ...
        <div className="p-6 space-y-6">
          {success ? (
            <div className="py-6 flex flex-col items-center text-center space-y-4">
              ...
              <div><h4 className="text-lg font-bold text-slate-900">Successfully Unloaded</h4><p className="text-sm text-slate-500 mt-1">Funds have been returned to your wallet.</p></div>
            </div>
          ) : (
            <>
              ...
              <button onClick={handleConfirm} disabled={!amount || parseFloat(amount) <= 0 || parseFloat(amount) > balNum || loading} className="w-full flex items-center justify-center gap-2 py-3 bg-emerald-600 text-white text-sm font-bold rounded-xl hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed">
                {loading ? <><Loader2 size={18} className="animate-spin" /> Processing...</> : 'Confirm Unload'}
              </button>
            </>
          )}
        </div>
