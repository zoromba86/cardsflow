...
export function AuthorReviewerBlock({
  ...
  return (
    <div className="w-full px-5 sm:px-12 lg:px-20 py-8 border-t border-zinc-800/50 mt-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-sm text-slate-500 max-w-4xl">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <UserCircle className="w-4 h-4 text-slate-500" />
            <span>Author: <span className="text-slate-700 font-medium">{authorName}</span></span>
          </div>
          {reviewerName && (
            <div className="flex items-center gap-2">
              <UserCircle className="w-4 h-4 text-slate-500" />
              <span>Reviewed by: <span className="text-slate-700 font-medium">{reviewerName}</span></span>
            </div>
          )}
        </div>
