...
function StatCard({ value, label, suffix = "" }: { value: number; label: string; suffix?: string }) {
  ...
  return (
    ...
    >
      <div className="text-5xl md:text-6xl font-black text-[#0F1B2D] tracking-tight">
        <motion.span>{count}</motion.span>
        <span>{suffix}</span>
      </div>
      <p className="text-zinc-500 font-medium text-base">{label}</p>
    </motion.div>
  );
}
...
const features = [
  ...
  },
  {
    ...
    checks: ["Detailed transaction & summary reports", "Excel and CSV multi-format export", "Automated monthly settlement assistant"],
  ...
  },
  {
    ...
    checks: ["Batch creation, activation, and freezing", "Customizable daily, monthly, and total limits", "Lifecycle managed from creation to destruction"],
    visual: "card-mgmt",
    reverse: false,
  },
  {
    ...
    checks: ["Scenario-based BIN selection engine", "95%+ payment success rate", "Chargeback and suspension risk control"],
    visual: "bin",
    reverse: true,
  },
];
    ...
    return (
      ...
      >
        <div className="relative w-80 h-80 flex items-center justify-center">
          {/* Glow rings */}
          <motion.div
            className="absolute inset-0 rounded-full border border-[#E5B220]/20"
            animate={{ scale: [1, 1.08, 1], opacity: [0.4, 0.1, 0.4] }}
            transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
          />
          <motion.div
            className="absolute inset-8 rounded-full border border-[#E5B220]/15"
            animate={{ scale: [1, 1.06, 1], opacity: [0.3, 0.08, 0.3] }}
            transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
          />
          {/* Center logo orbit */}
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-16 h-16 rounded-2xl bg-[#0F1B2D] flex items-center justify-center shadow-xl">
              <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
                <path d="M14 4L24 14L14 24L4 14L14 4Z" fill="#E5B220" />
              </svg>
            </div>
          </div>
          {/* Orbiting logos */}
          {platforms.map((p, i) => {
            ...
            return (
              <motion.div
                key={p.name}
                className="absolute"
                style={{ left: `calc(50% + ${x}px - 26px)`, top: `calc(50% + ${y}px - 26px)` }}
    ...
    return (
      ...
      >
        <div className="relative w-72 bg-white rounded-2xl shadow-2xl border border-zinc-100 overflow-hidden p-6">
          <motion.div
            className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#E5B220] to-[#0F1B2D]"
          ...
          {[
            { label: "Total Transactions", val: "$48,200", up: true },
            { label: "Success Rate", val: "96.4%", up: true },
            { label: "Chargebacks", val: "2", up: false },
            { label: "Active Cards", val: "138", up: true },
          ].map((row, i) => (
            <motion.div
              key={row.label}
          ...
          <motion.div
            className="mt-4 w-full bg-[#0F1B2D] text-white text-sm font-bold py-2.5 rounded-lg text-center"
          ...
          {cards.map((card, i) => (
            ...
            >
              <div className="flex justify-between items-center mb-4">
                ...
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${card.status === "Active" ? "bg-emerald-400/20 text-emerald-300" : "bg-white/10 text-white/50"}`}>
                  {card.status}
                </span>
              </div>
              <p className="font-mono text-lg font-bold tracking-widest">{card.num}</p>
              <div className="mt-3 flex justify-between text-xs text-white/40">
                <span>CARDSFLOW</span><span>VISA</span>
              </div>
            </motion.div>
          ))}
        </div>
  ...
  return (
    ...
    >
      <div className="relative w-72">
        <div className="bg-white border border-zinc-100 shadow-xl rounded-2xl p-5 overflow-hidden">
          ...
          <motion.div
            className="mt-4 h-2 rounded-full bg-zinc-100 overflow-hidden"
          >
            <motion.div
              className="h-full bg-gradient-to-r from-[#E5B220] to-[#0F1B2D] rounded-full"
              initial={{ width: 0 }}
  ...
  return (
    <section className="bg-background">
      ...
      <div className="border-y border-zinc-200 bg-white py-5 overflow-hidden">
        <motion.div
          className="flex items-center justify-center gap-12 text-sm font-semibold text-zinc-500"
          initial={{ opacity: 0, y: 10 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
        >
          <span className="uppercase tracking-widest text-xs">Works with</span>
          {platforms.map((p) => (
      ...
      <div className="max-w-5xl mx-auto px-6 py-24">
        ...
        >
          <StatCard value={95} suffix="%+" label="Payment Success" />
          <StatCard value={50} suffix="+" label="BIN Range Support" />
          <StatCard value={1000} suffix="+" label="Corporate Clients" />
          <StatCard value={24} suffix="/7" label="Tech Support" />
        </motion.div>

        {/* divider */}
        <motion.div
          className="mt-24 h-px bg-gradient-to-r from-transparent via-zinc-200 to-transparent"
          initial={{ scaleX: 0 }}
          whileInView={{ scaleX: 1 }}
          viewport={{ once: true }}
      ...
      <div className="max-w-7xl mx-auto px-6 pb-12">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-black text-[#0F1B2D] leading-tight mb-5">
            Everything your team needs<br />to scale ad spend
          </h2>
      ...
      {/* ── Feature rows ── */}
      <div className="max-w-7xl mx-auto px-6 pb-32 flex flex-col gap-32">
        {features.map((feat, idx) => {
          const textBlock = (
            ...
            >
              <motion.span
                className="inline-block text-[#E5B220] text-xs font-black uppercase tracking-[0.18em] mb-4 bg-[#E5B220]/10 px-3 py-1 rounded-full w-fit"
              ...
              <h3 className="text-3xl md:text-4xl font-black text-[#0F1B2D] leading-snug mb-5">{feat.title}</h3>
          ...
          const visual = (
            <div key="visual" className="flex-1 flex items-center justify-center">
          ...
          return (
            <div
              key={feat.title}
              className={`flex flex-col md:flex-row gap-16 items-center ${feat.reverse ? "md:flex-row-reverse" : ""}`}
            >
              {textBlock}
              {visual}
            </div>
